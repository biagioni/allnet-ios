...
- (void)assignContentForContact {
   ...
   display_offset = 0;
   for (NSObject * item in self.subviews) {
      if ([item isKindOfClass: [UITextView class]]) {
         UITextView * view = (UITextView *) item;
...
static UIColor * messageColor (struct message_store_info * info, uint64_t now) {
...
#define SECONDS_PER_DAY     (24 * 60 * 60)      // 86400
   if ((info->msg_type == MSG_TYPE_SENT) && (info->message_has_been_acked))
      ...
      if (info->rcvd_ackd_time + SECONDS_PER_DAY >= now) {
         if (now > info->rcvd_ackd_time)
            fractionOfDay = (((now - info->rcvd_ackd_time) * 1.0) / SECONDS_PER_DAY);
         else  // brand new message
            fractionOfDay = 0.0;
      }
      // NSLog(@"for message %s fraction of day is %f = (%llu - %llu) / %d\n", s.UTF8String, fractionOfDay, now, info->rcvd_ackd_time, SECONDS_PER_DAY);
      result = [[UIColor alloc] initWithRed:fractionOfDay green:1 blue:1 alpha:1];
   }
   return result;
}

static NSAttributedString * makeMessage (struct message_store_info * info, uint64_t now, int addNewlines) {
...
- (IBAction)messageFieldSelected:(id)sender {
   NSLog(@"ConversationUITextView.m: message field has been selected\n");
}
...
- (IBAction)messageEntered:(id)sender {
   ...
   if ((textView.text.length > 0) && (self.xcontact != NULL)) {  // don't send empty messages
      ...
      NSAttributedString * displayMessage = makeMessage (&info, info.time, 1);
...
- (void) addMessageToView:(struct message_store_info *) info time:(uint64_t)now {
   double selfWidth = self.superview.frame.size.width - 32;
   double bubbleWidth = selfWidth * 2.0 / 3.0;
   double selfHeight = self.contentSize.height;
   NSAttributedString * boxedString = makeMessage (info, now, 0);
   NSLog(@"boxedString is '%s'\n", boxedString.string.UTF8String);
   CGRect subf;
   subf.origin.x = ((info->msg_type != MSG_TYPE_SENT) ? 8 : (selfWidth - bubbleWidth - 8));
   subf.origin.y = selfHeight - 30;
   subf.size.height = 100;
   subf.size.width = bubbleWidth;
   UITextView * subv = [[UITextView alloc] initWithFrame:subf];
   subv.attributedText = [[NSMutableAttributedString alloc] initWithAttributedString: boxedString];
   [[subv layer] setCornerRadius:8.0];
   subf.size.height = subv.contentSize.height;
   subf.origin.y = display_offset;
   display_offset += subf.size.height + 8;
   // NSLog(@"resizing from %fx%f@%f@%f to %fx%f@%f@%f, self %fx%f@%f@%f .content size %fx%f intrinsic %fx%f, string '%s'\n", subv.frame.size.width, subv.frame.size.height, subv.frame.origin.x, subv.frame.origin.y, subf.size.width, subf.size.height, subf.origin.x, subf.origin.y, self.frame.size.width, self.frame.size.height, self.frame.origin.x, self.frame.origin.y, self.contentSize.width, self.contentSize.height, self.intrinsicContentSize.width, self.intrinsicContentSize.height, boxedString.string.UTF8String);
   [subv setFrame:subf];
   subv.backgroundColor = messageColor(info, now);
   [self addSubview:subv];
   CGSize csize = self.contentSize;
   if (csize.height < display_offset) {
      // NSLog(@"changing content height from %f to %d\n", csize.height, display_offset);
      csize.height = display_offset;
...
- (IBAction)messageEntered:(id)sender {
   ...
   if ((textView.text.length > 0) && (self.xcontact != NULL)) {  // don't send empty messages
      ...
      info.time = allnet_time();
      info.tz_min = local_time_offset();
      [self addMessageToView:&info time:info.time];
      textView.text = @"";
      NSLog (@"messageEntered calling scrollToEnd for %s\n", self.xcontact);
      scrollToEnd (self);
      [self setNeedsDisplay];
...
- (void)readyToShowKeyboard:(NSNotification *)notification {
   NSLog(@"readyToShowKeyboard, displayed %d\n", self.keyboardIsDisplayed);
   ...
   NSLog(@"initial superview frame %f %f %f %f\n", superViewFrame.origin.x, superViewFrame.origin.y, superViewFrame.size.width, superViewFrame.size.height);
   self.originalFrameHeight = superViewFrame.size.height;
   self.keyboardHeight = keyboardHeight;
   if (superViewFrame.size.height > keyboardHeight)
      superViewFrame.size.height -= keyboardHeight;
   else
      superViewFrame.size.height = 0;
   NSLog (@"frame height %d -> %d\n", self.originalFrameHeight, (int)superViewFrame.size.height);
   [self.superview setFrame:superViewFrame];
   self.keyboardIsDisplayed = YES;
   NSLog(@"set super view frame to %f %f %f %f, %@\n", superViewFrame.origin.x, superViewFrame.origin.y, superViewFrame.size.width, superViewFrame.size.height, self);
   scrollToEnd(self);
}

- (void)scrollForScreenChange:(NSNotification *)notification {
   NSLog(@"readytoScrollForScreenChange, displayed %d\n", self.keyboardIsDisplayed);
   if (self.keyboardIsDisplayed) {
...
- (void)readyToHideKeyboard:(NSNotification *)notification {
   NSLog(@"readyToHideKeyboard, displayed %d\n", self.keyboardIsDisplayed);
   if (! self.keyboardIsDisplayed)
      return;
   [self.messageField endEditing:YES];
   CGRect superViewFrame = self.superview.frame;
   NSLog(@"initial superview frame %f %f %f %f\n", superViewFrame.origin.x, superViewFrame.origin.y, superViewFrame.size.width, superViewFrame.size.height);
   superViewFrame.size.height = self.originalFrameHeight;
   NSLog (@"frame height %d -> %d\n", (int)superViewFrame.size.height, self.originalFrameHeight);
   [self.superview setFrame:superViewFrame];
   NSLog(@"hiding keyboard, original frame height %d, set frame to %f %f %f %f, sv %@\n", self.originalFrameHeight, superViewFrame.origin.x, superViewFrame.origin.y, superViewFrame.size.width, superViewFrame.size.height, self);
   self.keyboardIsDisplayed = NO;
   [self setNeedsDisplay];
}

- (void)enterBackgroundMode {
   NSLog(@"ConversationUITextView.m entering background mode, keyboard %d\n", self.keyboardIsDisplayed);
   [self readyToHideKeyboard:nil];
}
